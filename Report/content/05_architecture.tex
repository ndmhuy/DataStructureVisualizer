...
\begin{figure}[h!]
    ...
    % ================== MODEL ==================
    \node (ivisualizable) [box, fill=blue!10] {\textbf{IVisualizable}\\(Base Interface)};
    \node (structures) [box, fill=blue!5, below=of ivisualizable] {Data Structures\\(List, Graph, Heap, etc.)};
    
    \node (timeline) [box, fill=cyan!10, right=of ivisualizable, xshift=2cm] {Timeline \& Frames};
    ...
    % ================== CONTROLLER ==================
    \node (appEngine) [box, fill=green!10, below=of structures, yshift=-1.5cm] {\textbf{AppEngine}\\(Coordinator)};
    \node (dataManager) [box, fill=green!10, left=of appEngine, xshift=-1cm] {DataManager};
    \node (playback) [box, fill=green!10, right=of appEngine, xshift=1cm] {Playback};
    ...
    % ================== VIEW ==================
    \node (uimanager) [box, fill=red!10, below=of appEngine, yshift=-1.5cm, xshift=-2.5cm] {\textbf{UIManager}\\(ImGui)};
    \node (renderer) [box, fill=red!10, right=of uimanager, xshift=3cm] {\textbf{Renderer}\\(SFML)};
    
    \begin{scope}[on background layer]
        \node (viewGroup) [groupbox, fill=red!3, fit=(uimanager) (renderer)] {};
        \node [below left=0pt of viewGroup.south west, labelnode, color=red!70!black] {View Layer};
    \end{scope}
    
    % ================== UTILITIES ==================
    \node (utilities) [box, fill=yellow!10, left=of ivisualizable, xshift=-2cm] {Utilities\\(Pseudocode, config)};
    ...
    \draw [arrow] (appEngine) -- node[left, font=\scriptsize] {routes actions} (structures);
    \draw [dashed_arrow] (playback.east) -| node[right, font=\scriptsize, near start] {traverses} (timeline.south);
    
    % View connections
    \draw [arrow] (appEngine) -- node[left, font=\scriptsize] {updates} (uimanager);
    \draw [arrow] (appEngine) -- node[right, font=\scriptsize] {draws} (renderer);
    
    % Cross-layer interactions
    \draw [arrow] (renderer.east) -| ++(1.5cm, 0) |- node[right, font=\scriptsize, near start] {implements} (ipayloadvis.east);
    
    \draw [dashed_arrow] (renderer.north) -- node[right, font=\scriptsize, pos=0.8] {renders} (payloads.south);
    \draw [dashed_arrow] (uimanager.north) .. controls +(up:4.5) and +(down:2) .. node[left, font=\scriptsize, pos=0.8] {queries state} (timeline.south west);
    
    \draw [arrow] (uimanager) -| node[left, font=\scriptsize, pos=0.2] {reads context} (utilities);
    \end{tikzpicture}
    }
