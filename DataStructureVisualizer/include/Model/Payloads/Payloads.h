...
struct LinkedListPayload : public IPayload {
  ...
  std::map<std::string, size_t>
      pointers; // e.g., {"head": 0, "tail": 4, "current": 2}

  LinkedListPayload() = default;
  LinkedListPayload(std::vector<int> values,
                    std::vector<size_t> highlightedNodes = {},
                    std::map<std::string, size_t> pointers = {})
      : values(std::move(values)),
        highlightedNodes(std::move(highlightedNodes)),
...
struct TreePayload : public IPayload {
  ...
  TreePayload(std::vector<TreeNodeData> nodes, size_t rootId = 0,
              std::vector<size_t> highlightedNodes = {},
              std::map<std::string, size_t> pointers = {},
              std::vector<Position> positions = {})
      : nodes(std::move(nodes)), positions(std::move(positions)),
        rootId(rootId), highlightedNodes(std::move(highlightedNodes)),
        pointers(std::move(pointers)) {}
...
struct HeapPayload : public IPayload {
  ...
  HeapPayload() = default;
  HeapPayload(std::vector<int> arrayData,
              std::vector<size_t> highlightedIndices = {})
      : arrayData(std::move(arrayData)),
        highlightedIndices(std::move(highlightedIndices)) {}
...
struct GraphPayload : public IPayload {
  ...
  GraphPayload(std::vector<size_t> vertices, std::vector<Edge> edges = {},
               std::vector<size_t> highlightedVertices = {},
               std::vector<Edge> highlightedEdges = {},
               std::vector<Position> positions = {})
      : vertices(std::move(vertices)), positions(std::move(positions)),
        edges(std::move(edges)),
        highlightedVertices(std::move(highlightedVertices)),
        highlightedEdges(std::move(highlightedEdges)) {}
...
struct SingleSourcePayload : public IPayload {
  ...
  SingleSourcePayload(
      GraphPayload baseGraph, std::vector<int> distances = {},
      std::vector<size_t> previousVertices = {},
      std::vector<std::pair<int, size_t>> priorityQueueSnapShot = {})
      : baseGraph(std::move(baseGraph)), distances(std::move(distances)),
        previousVertices(std::move(previousVertices)),
        priorityQueueSnapShot(std::move(priorityQueueSnapShot)) {}
...
struct AStarPayload : public IPayload {
  ...
  AStarPayload(GraphPayload baseGraph, std::vector<int> gCosts = {},
               std::vector<int> hCosts = {}, std::vector<int> fCosts = {},
               std::vector<size_t> previousVertices = {},
               std::vector<std::pair<int, size_t>> priorityQueueSnapShot = {},
               size_t targetVertex = 0)
      : baseGraph(std::move(baseGraph)), gCosts(std::move(gCosts)),
        hCosts(std::move(hCosts)), fCosts(std::move(fCosts)),
        previousVertices(std::move(previousVertices)),
        priorityQueueSnapShot(std::move(priorityQueueSnapShot)),
...
struct AllPairsPayload : public IPayload {
  ...
  AllPairsPayload(GraphPayload baseGraph,
                  std::vector<std::vector<int>> distances = {},
                  std::vector<std::vector<size_t>> nextVertices = {},
                  std::vector<size_t> currentPath = {})
      : baseGraph(std::move(baseGraph)), distances(std::move(distances)),
        nextVertices(std::move(nextVertices)),
...
struct GridPayload : public IPayload {
  std::vector<std::vector<int>>
      gridData; // 0=Empty, 1=Wall, 2=Start, 3=Target, 4=Visited, 5=Path, etc
  std::pair<size_t, size_t> currentCell;

  GridPayload() = default;
  GridPayload(std::vector<std::vector<int>> gridData,
              std::pair<size_t, size_t> currentCell =
